...
class MyForm(QMainWindow):
	...
	def fileOpen(self):
		...
		if not(newName in self.fname) and ('.' in newName):
			...
			self.pushButtonStart.append(QPushButton("Start"))
			self.pushButtonPause.append(QPushButton("Pause"))
		
			self.horLayout.append(QHBoxLayout())
			self.horLayout1.append(QHBoxLayout())
			...
			self.horLayout1[self.tabQuant].addWidget(self.pushButtonStart[self.tabQuant])
			self.horLayout1[self.tabQuant].addWidget(self.pushButtonPause[self.tabQuant])
			
			self.layout[self.tabQuant].addLayout(self.horLayout[self.tabQuant], 0, 0, 7, 1)
			self.layout[self.tabQuant].addLayout(self.horLayout1[self.tabQuant], 7, 0, 1, 1)
	...
	def loadFile(self):
		if self.fname[self.tabQuant]:
			...
			print("self.fname[self.tabQuant]", self.fname[self.tabQuant])
			
			self.cvVideo.append(cv2.VideoCapture(self.fname[self.tabQuant]))
			ret, frame = self.cvVideo[self.tabQuant].read()


			print("self.cvVideo[self.tabQuant].read()", self.cvVideo[self.tabQuant].read())
	...
	def removeCurrentTab(self, currentIndex):
		...
		self.pushButtonStart.remove(self.pushButtonStart[currentIndex])
		self.pushButtonPause.remove(self.pushButtonPause[currentIndex])
	...
	def nextFrameSlot(self):
		...
		if ret == True:
			...
			if self.cropping[crntTab] == False:
				lim = len(self.vagas[crntTab])
			else:
				lim = len(self.vagas[crntTab])-1
			...
			if self.cropping[crntTab] == True:
				lastPos = len(self.vagas[crntTab])-1
				for pos in range(1, len(self.vagas[crntTab][lastPos])):
					cv2.line(self.cvImage[crntTab], self.vagas[crntTab][lastPos][pos-1], self.vagas[crntTab][lastPos][(pos)%4], (255, 255,0), 2)
